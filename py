...
class Agent():
    ...
    def learn(self,experiences , discount_factor):
        states,next_states,action,rewards,done = experiences
    ...
    def soft_update(self,local_modal,target_modal,interpolation_parameter):
        for target_param,local_param in zip(target_modal.parameters(),local_modal.parameters()):
...
for episode in range(1,number_episodes+1):
    ...
    if np.mean(scores_on_100_episodes) >= 200.0:
        print('\nEnvoronment solved in {:d} episodes! \tAverage Score: {:.2f}'.format(episode -100 , np.mean(scores_on_100_episodes)))
